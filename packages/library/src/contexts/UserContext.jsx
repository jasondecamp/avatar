...
const UserContextProvider = ({ children }) => {
  ...
  const exitProxy = () => userService.proxyEnd()
    .then(({user,...tokenData}) => {
      setToken(tokenData);
      window.localStorage.removeItem('state');
      if(hasRole('vendor')) return window.top.location = `${import.meta.env.VITE_MOVED_APP_URL}/admin/vendors`;
      if(hasRole('customer')) return window.top.location = `${import.meta.env.VITE_ADMIN_APP_URL}/cx/moves?direct=${encodeURIComponent(get(state,'user.email'))}`;
      if(hasRole('partner')) {
        const partner_id = get(get(state,'user.roles',[]).find(role => role.name === 'partner'),'partner_id');
        return window.top.location = `${import.meta.env.VITE_ADMIN_APP_URL}/admin/admins?partner_ids=${partner_id}`;
      }
    });
